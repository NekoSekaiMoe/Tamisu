...
import me.dabao1955.tamisu.ui.screen.getManagerVersion
import java.io.File
...
fun getBugreportFile(context: Context): File {
    ...
    shell.newJob().add("ORIG=\$(cat /proc/sys/core/kptr_restrict); echo 1 > /proc/sys/core/kptr_restrict; cat /proc/kallsyms > ${kallsyms.absolutePath}; echo \$ORIG > /proc/sys/core/kptr_restrict").exec()
    ...
    val targetFile = File(context.cacheDir, "Tamisu_bugreport_${current}.tar.gz")
