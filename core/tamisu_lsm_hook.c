...
int tamisu_lsm_hook(struct tamisu_lsm_hook *hook)
{
	...
	for (i = 0; i < scalls_count; i++) {
		...
		pr_info("found slot %zu orig %pSb\n", i, current_origin);
		...
		if (!hook->offset) {
			...
			pr_info("found real slot %zu orig %pSb\n", i,
				current_origin);
...

	{
		...
		pr_info("heads_addr 0x%lx head_offset 0x%lx heads_size %ld "
			"hook_offset 0x%lx\n",
			(unsigned long)heads_addr, hook->head_offset,
			heads_size, hook->hook_offset);
		...
		for (; head < head_end; head++) {
			hlist_for_each_entry(entry, head, list)
			{
				...
				if (current_origin == target) {
					pr_info(
					    "found %s (target %s) at head "
					    "offset %ld (provided %ld)\n",
					    hook->head_name, hook->target_name,
					    (unsigned long)head - heads_addr,
					    hook->head_offset);
					selected_entry = entry;
					selected_slot = slot;
					selected_origin = current_origin;
