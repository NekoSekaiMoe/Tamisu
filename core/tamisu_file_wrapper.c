...
struct tamisu_file_wrapper {
	struct file *orig;
	struct file_operations ops;
};

static struct tamisu_file_wrapper *tamisu_create_file_wrapper(struct file *fp);

static int tamisu_wrapper_open(struct inode *ino, struct file *fp)
{
...
static const struct file_operations tamisu_file_wrapper_inode_fops = {
    .owner = THIS_MODULE, .open = tamisu_wrapper_open};

static loff_t tamisu_wrapper_llseek(struct file *fp, loff_t off, int flags)
{
...
#else
static int tamisu_wrapper_iopoll(struct kiocb *kiocb, bool spin)
{
	struct tamisu_file_wrapper *data = kiocb->ki_filp->private_data;
	struct file *orig = data->orig;
	kiocb->ki_filp = orig;
...
static int tamisu_wrapper_iterate(struct file *fp, struct dir_context *dc)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
static int tamisu_wrapper_mmap(struct file *fp, struct vm_area_struct *vma)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
static int tamisu_wrapper_flush(struct file *fp, fl_owner_t id)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
static int tamisu_wrapper_fasync(int arg, struct file *fp, int arg2)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
static int tamisu_wrapper_lock(struct file *fp, int arg1, struct file_lock *fl)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
// static int tamisu_wrapper_check_flags(int arg) {}

static int tamisu_wrapper_flock(struct file *fp, int arg1, struct file_lock *fl)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
	if (orig->f_op->flock) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
void tamisu_wrapper_splice_eof(struct file *fp)
{
	struct tamisu_file_wrapper *data = fp->private_data;
	struct file *orig = data->orig;
...
static void tamisu_wrapper_show_fdinfo(struct seq_file *m, struct file *f)
{
	struct tamisu_file_wrapper *data = f->private_data;
	struct file *orig = data->orig;
...
// https://cs.android.com/android/core/superproject/+/common-android-mainline:common/fs/read_write.c;l=1593-1606;drc=398da7defe218d3e51b0f3bdff75147e28125b60
					      ...
					      unsigned int remap_flags)
{
	if (remap_flags & REMAP_FILE_DEDUP) {
		...
		struct tamisu_file_wrapper *data = file_in->private_data;
		struct file *orig = data->orig;
...
static void tamisu_release_file_wrapper(struct tamisu_file_wrapper *data);

static int tamisu_wrapper_release(struct inode *inode, struct file *filp)
{
	// https://cs.android.com/android/core/superproject/+/common-android-mainline:common/fs/file_table.c;l=467-473;drc=3be0b283b562eabbc2b1f3bb534dc8903079bbaa
	// f_op->release is called before fops_put(f_op), so we put it manually.
	fops_put(filp->f_op);
	// prevent it from being put again
	filp->f_op = NULL;
	tamisu_release_file_wrapper(filp->private_data);
	return 0;
}

static struct tamisu_file_wrapper *tamisu_create_file_wrapper(struct file *fp)
{
	struct tamisu_file_wrapper *p =
	    kcalloc(1, sizeof(struct tamisu_file_wrapper), GFP_KERNEL);
	if (!p) {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0)
	p->ops.iterate = fp->f_op->iterate ? tamisu_wrapper_iterate : NULL;
#endif // #if LINUX_VERSION_CODE < KERNEL_VERSION...
	p->ops.iterate_shared =
	    fp->f_op->iterate_shared ? tamisu_wrapper_iterate_shared : NULL;
	p->ops.poll = fp->f_op->poll ? tamisu_wrapper_poll : NULL;
	p->ops.unlocked_ioctl =
	    fp->f_op->unlocked_ioctl ? tamisu_wrapper_unlocked_ioctl : NULL;
	p->ops.compat_ioctl =
	    fp->f_op->compat_ioctl ? tamisu_wrapper_compat_ioctl : NULL;
	p->ops.mmap = fp->f_op->mmap ? tamisu_wrapper_mmap : NULL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
	p->ops.fop_flags = fp->f_op->fop_flags;
#else
	p->ops.mmap_supported_flags = fp->f_op->mmap_supported_flags;
#endif // #if LINUX_VERSION_CODE >= KERNEL_VERSIO...
	p->ops.flush = fp->f_op->flush ? tamisu_wrapper_flush : NULL;
	p->ops.release = tamisu_wrapper_release;
	p->ops.fsync = fp->f_op->fsync ? tamisu_wrapper_fsync : NULL;
	p->ops.fasync = fp->f_op->fasync ? tamisu_wrapper_fasync : NULL;
	p->ops.lock = fp->f_op->lock ? tamisu_wrapper_lock : NULL;
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0)
	...
	p->ops.splice_write =
	    fp->f_op->splice_write ? tamisu_wrapper_splice_write : NULL;
	p->ops.splice_read =
	...
	p->ops.copy_file_range =
	    fp->f_op->copy_file_range ? tamisu_wrapper_copy_file_range : NULL;
	p->ops.remap_file_range =
	    fp->f_op->remap_file_range ? tamisu_wrapper_remap_file_range : NULL;
	p->ops.fadvise = fp->f_op->fadvise ? tamisu_wrapper_fadvise : NULL;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	p->ops.splice_eof =
	    fp->f_op->splice_eof ? tamisu_wrapper_splice_eof : NULL;
#endif // #if LINUX_VERSION_CODE >= KERNEL_VERSIO...
...
static void tamisu_wrapper_d_release(struct dentry *dentry)
{
	struct path *orig_path = dentry->d_fsdata;
	path_put(orig_path);
	kfree(orig_path);
...
static const struct dentry_operations tamisu_file_wrapper_d_ops = {
    .d_dname = tamisu_wrapper_d_dname, .d_release = tamisu_wrapper_d_release};
...
// implement it.
// https://cs.android.com/android/core/superproject/+/common-android12-5.10:common/fs/anon_inodes.c;l=58-125;drc=0d34ce8aa78e38affbb501690bcabec4df88620e
...
static struct file *tamisu_anon_inode_create_getfile_compat(
    ...
    const struct inode *context_inode)
{
	...
	inode = tamisu_anon_inode_make_secure_inode(name, context_inode);
	if (IS_ERR(inode)) {
		file = ERR_CAST(inode);
		goto err;
	}
...
int tamisu_install_file_wrapper(int fd)
{
	...
	struct tamisu_file_wrapper *file_wrapper_data =
	    tamisu_create_file_wrapper(orig_file);
	...
	struct file *wrapper_file = tamisu_anon_inode_create_getfile_compat(
	    "[tamisu_fdwrapper]", &file_wrapper_data->ops, file_wrapper_data,
	    orig_file->f_flags, NULL);
	if (IS_ERR(wrapper_file)) {
		pr_err("tamisu_fdwrapper: getfile failed: %ld\n",
	...
	if (wrapper_sec) {
		wrapper_sec->sid = tamisu_file_sid;
	}
	// Install open file operation for inode.
	wrapper_inode->i_fop = &tamisu_file_wrapper_inode_fops;
	...
	*orig_path = orig_file->f_path;
	...
	wrapper_file->f_path.dentry->d_op = &tamisu_file_wrapper_d_ops;
...
out_put_wrapper_file:
	...
	goto out_put_fd;
out_release_wrapper:
	tamisu_release_file_wrapper(file_wrapper_data);
out_put_fd:
